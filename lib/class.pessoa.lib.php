<?php
/**
 * 📄 class.pessoa.lib.php - Classe para manipulação da entidade pessoa
 * 👤 Autor: Márcio Queiroz Jr <mqjunior@gmail.com> | 📅 2025-03-04 | 🏷️ v0.0.0
 * 📦 Pacote: pessoa | 📂 Subpacote: classe
 */
...
class Pessoa
{
    ...
    /**
     * @var    link $TBL_PESSOA Nome da Tabela da Entidade
     * @access private
     */
    private $TBL_PESSOA = null;
    /**
    ...
    public function __construct($p_SISTEMA)
    {
        ...
        $this->DataBaseConfig = $this->SISTEMA_['ENTIDADE']['PESSOA']['CONF']['DATABASE'];
        $this->TBL_PESSOA     = $this->DataBaseConfig['TBL_PESSOA'];
    ...
    {
        $this->ConectaDB();
        $sql_Alterar = "update " . $this->TBL_PESSOA . " set REG_ATIVO='0' where codigo='" . $p_Codigo . "'";
    ...
    {
        $this->ConectaDB();
        $sql_Alterar = "update " . $this->TBL_PESSOA . " set REG_ATIVO='1' where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Alterar);
        $this->Consultar($p_Codigo);
    }
    /**
    ...
    {
        $this->ConectaDB();
        $sql_Excluir = "DELETE FROM " . $this->TBL_PESSOA . " where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Excluir);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['PESSOA']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['PESSOA']['MENSAGEM']['SUCESSO']['EXCLUSAO'];
        $this->PesquisarNome();
    }
    /**
    ...
    public function Listar($p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao = "(Pessoa.REG_ATIVO=1)";
        if ($p_Inativos) {
            $sql_Condicao = "(1=1)";
        }

        $sql_Listar = "select  Pessoa.*
      FROM  " . $this->TBL_PESSOA . " as Pessoa
        where
          " . $sql_Condicao . "
      order by Pessoa.NOME " . $sql_QtdReg;

        $this->DataBaseLink->Query($sql_Listar);
        $this->SISTEMA_['ENTIDADE']['PESSOA']['DADOS'] = $this->DataBaseLink->ResultConsult();
    }
    /**
    ...
    public function PesquisarNome($p_NOME = '', $p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao .= " and (Pessoa.NOME like '" . $p_NOME . "%')";

        $sql_PesquisarNome = "select  Pessoa.*
      FROM  " . $this->TBL_PESSOA . " as Pessoa
        where
          " . $sql_Condicao . "
      order by Pessoa.NOME" . $sql_QtdReg;
    ...
    public function Pesquisar($p_CAMPO = 'NOME', $p_VALOR = '', $p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao .= " and (Pessoa.$p_CAMPO like '" . $p_VALOR . "%')";

        $sql_PesquisarNome = "select  Pessoa.*
      FROM  " . $this->TBL_PESSOA . " as Pessoa
        where
          " . $sql_Condicao . "
      order by Pessoa." . $p_CAMPO . $sql_QtdReg;
    ...
    {
        $this->ConectaDB();
        $sql_Consultar = "select Pessoa.*, Usuario.NOME_EXIBIR USUARIO_NOME
      FROM  " . $this->TBL_PESSOA . " as Pessoa
      Left join
          " . $this->TBL_USUARIO . " as Usuario on (Usuario.codigo = Pessoa.usuario)
        where
          Pessoa.codigo = '" . $p_Codigo . "'";

        $this->DataBaseLink->Query($sql_Consultar);
        $tmpDados                                     = $this->DataBaseLink->ResultConsult();
        $this->SISTEMA_['ENTIDADE']['PESSOA']['VARS'] = $tmpDados[0];
    }
    /**
    ...
    public function Incluir($p_Dados)
    {
        ...
        $this->DataBaseLink->Insert($this->TBL_PESSOA);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['PESSOA']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['PESSOA']['MENSAGEM']['SUCESSO']['MENSAGEM'];
    ...
    public function Alterar($p_Dados, $p_Codigo)
    {
        ...
        $sql_Alterar = "update " . $this->TBL_PESSOA . " set " . $sql_Complemento . " where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Alterar);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['PESSOA']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['PESSOA']['MENSAGEM']['SUCESSO']['MENSAGEM'];
        $this->Consultar($p_Codigo);
