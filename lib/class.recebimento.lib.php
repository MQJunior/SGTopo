<?php
/**
 * 📄 class.recebimento.lib.php - Classe para manipulação da entidade recebimento
 * 👤 Autor: Márcio Queiroz Jr <mqjunior@gmail.com> | 📅 2025-03-04 | 🏷️ v0.0.0
 * 📦 Pacote: recebimento | 📂 Subpacote: classe
 */
...
class Recebimento
{
    ...
    /**
     * @var    link $TBL_RECEBIMENTO Nome da Tabela da Entidade
     * @access private
     */
    private $TBL_RECEBIMENTO = null;
    /**
    ...
    public function __construct($p_SISTEMA)
    {
        ...
        $this->DataBaseConfig = $this->SISTEMA_['ENTIDADE']['RECEBIMENTO']['CONF']['DATABASE'];
        $this->TBL_RECEBIMENTO     = $this->DataBaseConfig['TBL_RECEBIMENTO'];
    ...
    {
        $this->ConectaDB();
        $sql_Alterar = "update " . $this->TBL_RECEBIMENTO . " set REG_ATIVO='0' where codigo='" . $p_Codigo . "'";
    ...
    {
        $this->ConectaDB();
        $sql_Alterar = "update " . $this->TBL_RECEBIMENTO . " set REG_ATIVO='1' where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Alterar);
        $this->Consultar($p_Codigo);
    }
    /**
    ...
    {
        $this->ConectaDB();
        $sql_Excluir = "DELETE FROM " . $this->TBL_RECEBIMENTO . " where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Excluir);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['RECEBIMENTO']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['RECEBIMENTO']['MENSAGEM']['SUCESSO']['EXCLUSAO'];
        $this->PesquisarNome();
    }
    /**
    ...
    public function Listar($p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao = "(Recebimento.REG_ATIVO=1)";
        if ($p_Inativos) {
            $sql_Condicao = "(1=1)";
        }

        $sql_Listar = "select  Recebimento.*
      FROM  " . $this->TBL_RECEBIMENTO . " as Recebimento
        where
          " . $sql_Condicao . "
      order by Recebimento.NOME " . $sql_QtdReg;

        $this->DataBaseLink->Query($sql_Listar);
        $this->SISTEMA_['ENTIDADE']['RECEBIMENTO']['DADOS'] = $this->DataBaseLink->ResultConsult();
    }
    /**
    ...
    public function PesquisarNome($p_NOME = '', $p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao .= " and (Recebimento.NOME like '" . $p_NOME . "%')";

        $sql_PesquisarNome = "select  Recebimento.*
      FROM  " . $this->TBL_RECEBIMENTO . " as Recebimento
        where
          " . $sql_Condicao . "
      order by Recebimento.NOME" . $sql_QtdReg;
    ...
    public function Pesquisar($p_CAMPO = 'NOME', $p_VALOR = '', $p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao .= " and (Recebimento.$p_CAMPO like '" . $p_VALOR . "%')";

        $sql_PesquisarNome = "select  Recebimento.*
      FROM  " . $this->TBL_RECEBIMENTO . " as Recebimento
        where
          " . $sql_Condicao . "
      order by Recebimento." . $p_CAMPO . $sql_QtdReg;
    ...
    {
        $this->ConectaDB();
        $sql_Consultar = "select Recebimento.*, Usuario.NOME_EXIBIR USUARIO_NOME
      FROM  " . $this->TBL_RECEBIMENTO . " as Recebimento
      Left join
          " . $this->TBL_USUARIO . " as Usuario on (Usuario.codigo = Recebimento.usuario)
        where
          Recebimento.codigo = '" . $p_Codigo . "'";

        $this->DataBaseLink->Query($sql_Consultar);
        $tmpDados                                     = $this->DataBaseLink->ResultConsult();
        $this->SISTEMA_['ENTIDADE']['RECEBIMENTO']['VARS'] = $tmpDados[0];
    }
    /**
    ...
    public function Incluir($p_Dados)
    {
        ...
        $this->DataBaseLink->Insert($this->TBL_RECEBIMENTO);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['RECEBIMENTO']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['RECEBIMENTO']['MENSAGEM']['SUCESSO']['MENSAGEM'];
    ...
    public function Alterar($p_Dados, $p_Codigo)
    {
        ...
        $sql_Alterar = "update " . $this->TBL_RECEBIMENTO . " set " . $sql_Complemento . " where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Alterar);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['RECEBIMENTO']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['RECEBIMENTO']['MENSAGEM']['SUCESSO']['MENSAGEM'];
        $this->Consultar($p_Codigo);
