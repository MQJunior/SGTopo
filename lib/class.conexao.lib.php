...
//			return $this->MENSAGEMDEBUG;
	...
	public function ListarTabelas()
	{
		if ($this->TipoDB == 0) { // MySQL
			...
			$i = 0;
			foreach ($tmpRetorno as $Retorno)
				$Resultado[$i++]['NOME'] = $Retorno[0];
	...
	public function ListarCamposTabela($p_tabela)
	{
		if ($this->TipoDB == 0) { // MySQL
			...
			foreach ($tmpRetorno as $tmpColunas => $tmpCampos) {
				...
				if ($tmpCampos[2] == "NO")
					$_tmpRetorno['REQUERIDO'] = 1;

				// Definir Tipo

				//$_tmpRetorno['TIPO'] = $tmpCampos[1];
				if (($tmpCampos[1] == "int") && ($tmpCampos["Key"] == "MUL"))
					$_tmpRetorno["DOMINIO"] = "CODIGO_LINK";
				if (($tmpCampos[1] == "int") && ($tmpCampos["Key"] == "PRI"))
					$_tmpRetorno["DOMINIO"] = "CODIGO";
				if (($_tmpRetorno['CAMPO'] == "NOME") || ($tmpCampos[1] == "varchar(100)"))
					$_tmpRetorno["DOMINIO"] = "NOME";
				if ($tmpCampos[1] == "varchar(10)")
					$_tmpRetorno["DOMINIO"] = "NOME_CURTO";
				if ($tmpCampos[1] == "varchar(150)")
					$_tmpRetorno["DOMINIO"] = "DESCRICAO";
				if ($tmpCampos[1] == "varchar(150)")
					$_tmpRetorno["DOMINIO"] = "DESCRICAO";
				if (($tmpCampos[1] == "varchar(1000)") || ($tmpCampos[1] == "text"))
					$_tmpRetorno["DOMINIO"] = "NOME_TEXTO";
				if ($tmpCampos[1] == "datetime")
					$_tmpRetorno["DOMINIO"] = "DATA";
