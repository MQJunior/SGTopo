...
class permissao
{
    ...
    public function __construct($p_SISTEMA)
    {
        ...
        $tmp_idioma = strtolower($this->SISTEMA_['CONFIG']['SISTEMA']['GERAL']['IDIOMA']);
        require_once $this->SISTEMA_['CONFIG']['SISTEMA']['GERAL']['DEF'] . "permissao/permissao.idioma.{$tmp_idioma}.def.php";
    ...
    order by " . $this->TBL_ACAO . ".ENTIDADE, " . $this->TBL_ACAO . ".NOME, " . $this->TBL_ACAO . ".RESTRITO";
        ...
        foreach ($tmpDados as $listaPermissao) {
            $dadosSaida[$listaPermissao['ENTIDADE']][] = ["CODIGO" => $listaPermissao['CODIGO'], "ACAO" => $listaPermissao['ACAO'], "RESTRITO" => $listaPermissao['RESTRITO']];
        }
        $tmpDados = $dadosSaida;
   ...
   " . $this->TBL_ACAO . ".RESTRITO=0)
    ...
    public function ChecarPermissaoSys()
    {
        ...
        if (! $this->ChecarPermissao($tmpCodigoUsuario, $tmpEntidade, $tmpAcao)) {
            ...
            $this->SISTEMA_['ERROR']['PERMISSAO']['MENSAGEM']    = $this->SISTEMA_['ENTIDADE']['PERMISSAO']['MENSAGEM']['ERROR']['PERMISSAO'] . ": " . $tmpEntidade . " -> " . $tmpAcao;
