<?php
/**
 * 📄 class.servico.lib.php - Classe para manipulação da entidade servico
 * 👤 Autor: Márcio Queiroz Jr <mqjunior@gmail.com> | 📅 2025-03-04 | 🏷️ v0.0.0
 * 📦 Pacote: servico | 📂 Subpacote: classe
 */
...
class Servico
{
    ...
    /**
     * @var    link $TBL_SERVICO Nome da Tabela da Entidade
     * @access private
     */
    private $TBL_SERVICO = null;
    /**
    ...
    public function __construct($p_SISTEMA)
    {
        ...
        $this->DataBaseConfig = $this->SISTEMA_['ENTIDADE']['SERVICO']['CONF']['DATABASE'];
        $this->TBL_SERVICO     = $this->DataBaseConfig['TBL_SERVICO'];
    ...
    {
        $this->ConectaDB();
        $sql_Alterar = "update " . $this->TBL_SERVICO . " set REG_ATIVO='0' where codigo='" . $p_Codigo . "'";
    ...
    {
        $this->ConectaDB();
        $sql_Alterar = "update " . $this->TBL_SERVICO . " set REG_ATIVO='1' where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Alterar);
        $this->Consultar($p_Codigo);
    }
    /**
    ...
    {
        $this->ConectaDB();
        $sql_Excluir = "DELETE FROM " . $this->TBL_SERVICO . " where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Excluir);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['SERVICO']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['SERVICO']['MENSAGEM']['SUCESSO']['EXCLUSAO'];
        $this->PesquisarNome();
    }
    /**
    ...
    public function Listar($p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao = "(Servico.REG_ATIVO=1)";
        if ($p_Inativos) {
            $sql_Condicao = "(1=1)";
        }

        $sql_Listar = "select  Servico.*
      FROM  " . $this->TBL_SERVICO . " as Servico
        where
          " . $sql_Condicao . "
      order by Servico.NOME " . $sql_QtdReg;

        $this->DataBaseLink->Query($sql_Listar);
        $this->SISTEMA_['ENTIDADE']['SERVICO']['DADOS'] = $this->DataBaseLink->ResultConsult();
    }
    /**
    ...
    public function PesquisarNome($p_NOME = '', $p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao .= " and (Servico.NOME like '" . $p_NOME . "%')";

        $sql_PesquisarNome = "select  Servico.*
      FROM  " . $this->TBL_SERVICO . " as Servico
        where
          " . $sql_Condicao . "
      order by Servico.NOME" . $sql_QtdReg;
    ...
    public function Pesquisar($p_CAMPO = 'NOME', $p_VALOR = '', $p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao .= " and (Servico.$p_CAMPO like '" . $p_VALOR . "%')";

        $sql_PesquisarNome = "select  Servico.*
      FROM  " . $this->TBL_SERVICO . " as Servico
        where
          " . $sql_Condicao . "
      order by Servico." . $p_CAMPO . $sql_QtdReg;
    ...
    {
        $this->ConectaDB();
        $sql_Consultar = "select Servico.*, Usuario.NOME_EXIBIR USUARIO_NOME
      FROM  " . $this->TBL_SERVICO . " as Servico
      Left join
          " . $this->TBL_USUARIO . " as Usuario on (Usuario.codigo = Servico.usuario)
        where
          Servico.codigo = '" . $p_Codigo . "'";

        $this->DataBaseLink->Query($sql_Consultar);
        $tmpDados                                     = $this->DataBaseLink->ResultConsult();
        $this->SISTEMA_['ENTIDADE']['SERVICO']['VARS'] = $tmpDados[0];
    }
    /**
    ...
    public function Incluir($p_Dados)
    {
        ...
        $this->DataBaseLink->Insert($this->TBL_SERVICO);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['SERVICO']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['SERVICO']['MENSAGEM']['SUCESSO']['MENSAGEM'];
    ...
    public function Alterar($p_Dados, $p_Codigo)
    {
        ...
        $sql_Alterar = "update " . $this->TBL_SERVICO . " set " . $sql_Complemento . " where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Alterar);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['SERVICO']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['SERVICO']['MENSAGEM']['SUCESSO']['MENSAGEM'];
        $this->Consultar($p_Codigo);
