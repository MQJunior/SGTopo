<?php
/**
 * 📄 class.arquivos.lib.php - Classe para manipulação da entidade arquivos
 * 👤 Autor: Márcio Queiroz Jr <mqjunior@gmail.com> | 📅 2025-03-04 | 🏷️ v0.0.0
 * 📦 Pacote: arquivos | 📂 Subpacote: classe
 */
...
class Arquivos
{
    ...
    /**
     * @var    link $TBL_ARQUIVOS Nome da Tabela da Entidade
     * @access private
     */
    private $TBL_ARQUIVOS = null;
    /**
    ...
    public function __construct($p_SISTEMA)
    {
        ...
        $this->DataBaseConfig = $this->SISTEMA_['ENTIDADE']['ARQUIVOS']['CONF']['DATABASE'];
        $this->TBL_ARQUIVOS     = $this->DataBaseConfig['TBL_ARQUIVOS'];
    ...
    {
        $this->ConectaDB();
        $sql_Alterar = "update " . $this->TBL_ARQUIVOS . " set REG_ATIVO='0' where codigo='" . $p_Codigo . "'";
    ...
    {
        $this->ConectaDB();
        $sql_Alterar = "update " . $this->TBL_ARQUIVOS . " set REG_ATIVO='1' where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Alterar);
        $this->Consultar($p_Codigo);
    }
    /**
    ...
    {
        $this->ConectaDB();
        $sql_Excluir = "DELETE FROM " . $this->TBL_ARQUIVOS . " where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Excluir);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['ARQUIVOS']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['ARQUIVOS']['MENSAGEM']['SUCESSO']['EXCLUSAO'];
        $this->PesquisarNome();
    }
    /**
    ...
    public function Listar($p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao = "(Arquivos.REG_ATIVO=1)";
        if ($p_Inativos) {
            $sql_Condicao = "(1=1)";
        }

        $sql_Listar = "select  Arquivos.*
      FROM  " . $this->TBL_ARQUIVOS . " as Arquivos
        where
          " . $sql_Condicao . "
      order by Arquivos.NOME " . $sql_QtdReg;

        $this->DataBaseLink->Query($sql_Listar);
        $this->SISTEMA_['ENTIDADE']['ARQUIVOS']['DADOS'] = $this->DataBaseLink->ResultConsult();
    }
    /**
    ...
    public function PesquisarNome($p_NOME = '', $p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao .= " and (Arquivos.NOME like '" . $p_NOME . "%')";

        $sql_PesquisarNome = "select  Arquivos.*
      FROM  " . $this->TBL_ARQUIVOS . " as Arquivos
        where
          " . $sql_Condicao . "
      order by Arquivos.NOME" . $sql_QtdReg;
    ...
    public function Pesquisar($p_CAMPO = 'NOME', $p_VALOR = '', $p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao .= " and (Arquivos.$p_CAMPO like '" . $p_VALOR . "%')";

        $sql_PesquisarNome = "select  Arquivos.*
      FROM  " . $this->TBL_ARQUIVOS . " as Arquivos
        where
          " . $sql_Condicao . "
      order by Arquivos." . $p_CAMPO . $sql_QtdReg;
    ...
    {
        $this->ConectaDB();
        $sql_Consultar = "select Arquivos.*, Usuario.NOME_EXIBIR USUARIO_NOME
      FROM  " . $this->TBL_ARQUIVOS . " as Arquivos
      Left join
          " . $this->TBL_USUARIO . " as Usuario on (Usuario.codigo = Arquivos.usuario)
        where
          Arquivos.codigo = '" . $p_Codigo . "'";

        $this->DataBaseLink->Query($sql_Consultar);
        $tmpDados                                     = $this->DataBaseLink->ResultConsult();
        $this->SISTEMA_['ENTIDADE']['ARQUIVOS']['VARS'] = $tmpDados[0];
    }
    /**
    ...
    public function Incluir($p_Dados)
    {
        ...
        $this->DataBaseLink->Insert($this->TBL_ARQUIVOS);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['ARQUIVOS']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['ARQUIVOS']['MENSAGEM']['SUCESSO']['MENSAGEM'];
    ...
    public function Alterar($p_Dados, $p_Codigo)
    {
        ...
        $sql_Alterar = "update " . $this->TBL_ARQUIVOS . " set " . $sql_Complemento . " where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Alterar);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['ARQUIVOS']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['ARQUIVOS']['MENSAGEM']['SUCESSO']['MENSAGEM'];
        $this->Consultar($p_Codigo);
