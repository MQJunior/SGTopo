<?php
/**
 * 📄 class.solicitante.lib.php - Classe para manipulação da entidade solicitante
 * 👤 Autor: Márcio Queiroz Jr <mqjunior@gmail.com> | 📅 2025-03-04 | 🏷️ v0.0.0
 * 📦 Pacote: solicitante | 📂 Subpacote: classe
 */
...
class Solicitante
{
    ...
    /**
     * @var    link $TBL_SOLICITANTE Nome da Tabela da Entidade
     * @access private
     */
    private $TBL_SOLICITANTE = null;
    /**
    ...
    public function __construct($p_SISTEMA)
    {
        ...
        $this->DataBaseConfig = $this->SISTEMA_['ENTIDADE']['SOLICITANTE']['CONF']['DATABASE'];
        $this->TBL_SOLICITANTE     = $this->DataBaseConfig['TBL_SOLICITANTE'];
    ...
    {
        $this->ConectaDB();
        $sql_Alterar = "update " . $this->TBL_SOLICITANTE . " set REG_ATIVO='0' where codigo='" . $p_Codigo . "'";
    ...
    {
        $this->ConectaDB();
        $sql_Alterar = "update " . $this->TBL_SOLICITANTE . " set REG_ATIVO='1' where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Alterar);
        $this->Consultar($p_Codigo);
    }
    /**
    ...
    {
        $this->ConectaDB();
        $sql_Excluir = "DELETE FROM " . $this->TBL_SOLICITANTE . " where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Excluir);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['SOLICITANTE']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['SOLICITANTE']['MENSAGEM']['SUCESSO']['EXCLUSAO'];
        $this->PesquisarNome();
    }
    /**
    ...
    public function Listar($p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao = "(Solicitante.REG_ATIVO=1)";
        if ($p_Inativos) {
            $sql_Condicao = "(1=1)";
        }

        $sql_Listar = "select  Solicitante.*
      FROM  " . $this->TBL_SOLICITANTE . " as Solicitante
        where
          " . $sql_Condicao . "
      order by Solicitante.NOME " . $sql_QtdReg;

        $this->DataBaseLink->Query($sql_Listar);
        $this->SISTEMA_['ENTIDADE']['SOLICITANTE']['DADOS'] = $this->DataBaseLink->ResultConsult();
    }
    /**
    ...
    public function PesquisarNome($p_NOME = '', $p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao .= " and (Solicitante.NOME like '" . $p_NOME . "%')";

        $sql_PesquisarNome = "select  Solicitante.*
      FROM  " . $this->TBL_SOLICITANTE . " as Solicitante
        where
          " . $sql_Condicao . "
      order by Solicitante.NOME" . $sql_QtdReg;
    ...
    public function Pesquisar($p_CAMPO = 'NOME', $p_VALOR = '', $p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao .= " and (Solicitante.$p_CAMPO like '" . $p_VALOR . "%')";

        $sql_PesquisarNome = "select  Solicitante.*
      FROM  " . $this->TBL_SOLICITANTE . " as Solicitante
        where
          " . $sql_Condicao . "
      order by Solicitante." . $p_CAMPO . $sql_QtdReg;
    ...
    {
        $this->ConectaDB();
        $sql_Consultar = "select Solicitante.*, Usuario.NOME_EXIBIR USUARIO_NOME
      FROM  " . $this->TBL_SOLICITANTE . " as Solicitante
      Left join
          " . $this->TBL_USUARIO . " as Usuario on (Usuario.codigo = Solicitante.usuario)
        where
          Solicitante.codigo = '" . $p_Codigo . "'";

        $this->DataBaseLink->Query($sql_Consultar);
        $tmpDados                                     = $this->DataBaseLink->ResultConsult();
        $this->SISTEMA_['ENTIDADE']['SOLICITANTE']['VARS'] = $tmpDados[0];
    }
    /**
    ...
    public function Incluir($p_Dados)
    {
        ...
        $this->DataBaseLink->Insert($this->TBL_SOLICITANTE);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['SOLICITANTE']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['SOLICITANTE']['MENSAGEM']['SUCESSO']['MENSAGEM'];
    ...
    public function Alterar($p_Dados, $p_Codigo)
    {
        ...
        $sql_Alterar = "update " . $this->TBL_SOLICITANTE . " set " . $sql_Complemento . " where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Alterar);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['SOLICITANTE']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['SOLICITANTE']['MENSAGEM']['SUCESSO']['MENSAGEM'];
        $this->Consultar($p_Codigo);
