<?php
/**
 * 📄 class.tipodocumento.lib.php - Classe para manipulação da entidade tipodocumento
 * 👤 Autor: Márcio Queiroz Jr <mqjunior@gmail.com> | 📅 2025-03-04 | 🏷️ v0.0.0
 * 📦 Pacote: tipodocumento | 📂 Subpacote: classe
 */
...
class Tipodocumento
{
    ...
    /**
     * @var    link $TBL_TIPODOCUMENTO Nome da Tabela da Entidade
     * @access private
     */
    private $TBL_TIPODOCUMENTO = null;
    /**
    ...
    public function __construct($p_SISTEMA)
    {
        ...
        $this->DataBaseConfig = $this->SISTEMA_['ENTIDADE']['TIPODOCUMENTO']['CONF']['DATABASE'];
        $this->TBL_TIPODOCUMENTO     = $this->DataBaseConfig['TBL_TIPODOCUMENTO'];
    ...
    {
        $this->ConectaDB();
        $sql_Alterar = "update " . $this->TBL_TIPODOCUMENTO . " set REG_ATIVO='0' where codigo='" . $p_Codigo . "'";
    ...
    {
        $this->ConectaDB();
        $sql_Alterar = "update " . $this->TBL_TIPODOCUMENTO . " set REG_ATIVO='1' where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Alterar);
        $this->Consultar($p_Codigo);
    }
    /**
    ...
    {
        $this->ConectaDB();
        $sql_Excluir = "DELETE FROM " . $this->TBL_TIPODOCUMENTO . " where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Excluir);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['TIPODOCUMENTO']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['TIPODOCUMENTO']['MENSAGEM']['SUCESSO']['EXCLUSAO'];
        $this->PesquisarNome();
    }
    /**
    ...
    public function Listar($p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao = "(Tipodocumento.REG_ATIVO=1)";
        if ($p_Inativos) {
            $sql_Condicao = "(1=1)";
        }

        $sql_Listar = "select  Tipodocumento.*
      FROM  " . $this->TBL_TIPODOCUMENTO . " as Tipodocumento
        where
          " . $sql_Condicao . "
      order by Tipodocumento.NOME " . $sql_QtdReg;

        $this->DataBaseLink->Query($sql_Listar);
        $this->SISTEMA_['ENTIDADE']['TIPODOCUMENTO']['DADOS'] = $this->DataBaseLink->ResultConsult();
    }
    /**
    ...
    public function PesquisarNome($p_NOME = '', $p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao .= " and (Tipodocumento.NOME like '" . $p_NOME . "%')";

        $sql_PesquisarNome = "select  Tipodocumento.*
      FROM  " . $this->TBL_TIPODOCUMENTO . " as Tipodocumento
        where
          " . $sql_Condicao . "
      order by Tipodocumento.NOME" . $sql_QtdReg;
    ...
    public function Pesquisar($p_CAMPO = 'NOME', $p_VALOR = '', $p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao .= " and (Tipodocumento.$p_CAMPO like '" . $p_VALOR . "%')";

        $sql_PesquisarNome = "select  Tipodocumento.*
      FROM  " . $this->TBL_TIPODOCUMENTO . " as Tipodocumento
        where
          " . $sql_Condicao . "
      order by Tipodocumento." . $p_CAMPO . $sql_QtdReg;
    ...
    {
        $this->ConectaDB();
        $sql_Consultar = "select Tipodocumento.*, Usuario.NOME_EXIBIR USUARIO_NOME
      FROM  " . $this->TBL_TIPODOCUMENTO . " as Tipodocumento
      Left join
          " . $this->TBL_USUARIO . " as Usuario on (Usuario.codigo = Tipodocumento.usuario)
        where
          Tipodocumento.codigo = '" . $p_Codigo . "'";

        $this->DataBaseLink->Query($sql_Consultar);
        $tmpDados                                     = $this->DataBaseLink->ResultConsult();
        $this->SISTEMA_['ENTIDADE']['TIPODOCUMENTO']['VARS'] = $tmpDados[0];
    }
    /**
    ...
    public function Incluir($p_Dados)
    {
        ...
        $this->DataBaseLink->Insert($this->TBL_TIPODOCUMENTO);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['TIPODOCUMENTO']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['TIPODOCUMENTO']['MENSAGEM']['SUCESSO']['MENSAGEM'];
    ...
    public function Alterar($p_Dados, $p_Codigo)
    {
        ...
        $sql_Alterar = "update " . $this->TBL_TIPODOCUMENTO . " set " . $sql_Complemento . " where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Alterar);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['TIPODOCUMENTO']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['TIPODOCUMENTO']['MENSAGEM']['SUCESSO']['MENSAGEM'];
        $this->Consultar($p_Codigo);
