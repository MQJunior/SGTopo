<?php
/**
 * 📄 class.imovel.lib.php - Classe para manipulação da entidade imovel
 * 👤 Autor: Márcio Queiroz Jr <mqjunior@gmail.com> | 📅 2025-03-04 | 🏷️ v0.0.0
 * 📦 Pacote: imovel | 📂 Subpacote: classe
 */
...
class Imovel
{
    ...
    /**
     * @var    link $TBL_IMOVEL Nome da Tabela da Entidade
     * @access private
     */
    private $TBL_IMOVEL = null;
    /**
    ...
    public function __construct($p_SISTEMA)
    {
        ...
        $this->DataBaseConfig = $this->SISTEMA_['ENTIDADE']['IMOVEL']['CONF']['DATABASE'];
        $this->TBL_IMOVEL     = $this->DataBaseConfig['TBL_IMOVEL'];
    ...
    {
        $this->ConectaDB();
        $sql_Alterar = "update " . $this->TBL_IMOVEL . " set REG_ATIVO='0' where codigo='" . $p_Codigo . "'";
    ...
    {
        $this->ConectaDB();
        $sql_Alterar = "update " . $this->TBL_IMOVEL . " set REG_ATIVO='1' where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Alterar);
        $this->Consultar($p_Codigo);
    }
    /**
    ...
    {
        $this->ConectaDB();
        $sql_Excluir = "DELETE FROM " . $this->TBL_IMOVEL . " where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Excluir);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['IMOVEL']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['IMOVEL']['MENSAGEM']['SUCESSO']['EXCLUSAO'];
        $this->PesquisarNome();
    }
    /**
    ...
    public function Listar($p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao = "(Imovel.REG_ATIVO=1)";
        if ($p_Inativos) {
            $sql_Condicao = "(1=1)";
        }

        $sql_Listar = "select  Imovel.*
      FROM  " . $this->TBL_IMOVEL . " as Imovel
        where
          " . $sql_Condicao . "
      order by Imovel.NOME " . $sql_QtdReg;

        $this->DataBaseLink->Query($sql_Listar);
        $this->SISTEMA_['ENTIDADE']['IMOVEL']['DADOS'] = $this->DataBaseLink->ResultConsult();
    }
    /**
    ...
    public function PesquisarNome($p_NOME = '', $p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao .= " and (Imovel.NOME like '" . $p_NOME . "%')";

        $sql_PesquisarNome = "select  Imovel.*
      FROM  " . $this->TBL_IMOVEL . " as Imovel
        where
          " . $sql_Condicao . "
      order by Imovel.NOME" . $sql_QtdReg;
    ...
    public function Pesquisar($p_CAMPO = 'NOME', $p_VALOR = '', $p_Inativos = false, $p_QtdeReg = null)
    {
        ...
        $sql_Condicao .= " and (Imovel.$p_CAMPO like '" . $p_VALOR . "%')";

        $sql_PesquisarNome = "select  Imovel.*
      FROM  " . $this->TBL_IMOVEL . " as Imovel
        where
          " . $sql_Condicao . "
      order by Imovel." . $p_CAMPO . $sql_QtdReg;
    ...
    {
        $this->ConectaDB();
        $sql_Consultar = "select Imovel.*, Usuario.NOME_EXIBIR USUARIO_NOME
      FROM  " . $this->TBL_IMOVEL . " as Imovel
      Left join
          " . $this->TBL_USUARIO . " as Usuario on (Usuario.codigo = Imovel.usuario)
        where
          Imovel.codigo = '" . $p_Codigo . "'";

        $this->DataBaseLink->Query($sql_Consultar);
        $tmpDados                                     = $this->DataBaseLink->ResultConsult();
        $this->SISTEMA_['ENTIDADE']['IMOVEL']['VARS'] = $tmpDados[0];
    }
    /**
    ...
    public function Incluir($p_Dados)
    {
        ...
        $this->DataBaseLink->Insert($this->TBL_IMOVEL);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['IMOVEL']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['IMOVEL']['MENSAGEM']['SUCESSO']['MENSAGEM'];
    ...
    public function Alterar($p_Dados, $p_Codigo)
    {
        ...
        $sql_Alterar = "update " . $this->TBL_IMOVEL . " set " . $sql_Complemento . " where codigo='" . $p_Codigo . "'";
        $this->DataBaseLink->Query($sql_Alterar);
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['TITULO']   = $this->SISTEMA_['ENTIDADE']['IMOVEL']['MENSAGEM']['SUCESSO']['TITULO'];
        $this->SISTEMA_['MENSAGEM']['SUCESSO']['MENSAGEM'] = $this->SISTEMA_['ENTIDADE']['IMOVEL']['MENSAGEM']['SUCESSO']['MENSAGEM'];
        $this->Consultar($p_Codigo);
