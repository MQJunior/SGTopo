...
if (is_array($VAR_SISTEMA_TABELAS_CAMPOS)) {
    $tmpCamposNao = ['CODIGO', 'SESSAO', 'DATACRIACAO', 'USUARIO', 'REG_ATIVO'];
    foreach ($VAR_SISTEMA_TABELAS_CAMPOS as $tmpCampos) {
        if (! in_array($tmpCampos['CAMPO'], $tmpCamposNao)) {
            ...
            $tmpScriptSaida .= "\$Entidade" . ucwords(strtolower($tmpEntidade)) . "Campos['TXT_" . strtoupper($tmpEntidade) . "_" . $tmpcampoUPPER . "']=array('NOME' =>'$tmpcampoUPPER'     , 'TIPO'=>'$tmpcampoDominio'    , 'EXIBIR'=>true , 'PESQUISAR'=>true , 'REQUERIDO'=>$tmpcampoRequerido , 'TAMANHO'=>$tmpcampoTamanho);\n";
        }
    }
